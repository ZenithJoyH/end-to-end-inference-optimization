...
修复后先用精度评测 Skill 的 `minimal-regression` 覆盖复现与边界矩阵；原问题来自正式 GPQA 时必须对固定后的候选重新执行 `formal-gate`。相关重跑复用精度评测 Skill 的进度 heartbeat，不重复创建监控。当前定位状态和带稳定 issue ID 的历史摘要写入 `optimize/state.yml`，人可读结论同步到 `optimize/README.md`，不得另建历史目录；只有原失败范围重新通过后才恢复性能优化。
...
下一轮规划通常只保留 1–3 个候选，按端到端瓶颈贡献、可消除比例、生产命中率、证据强度、实现/验证成本和正确性/兼容风险排序。每个候选必须注明分类、适用边界、支持/反对证据、预期主/守护指标、依赖、最小证伪实验、场景与评测模式、成功门槛、停止条件和回退。当前结果、旧决策摘要和下一次复盘触发条件统一更新到 `optimize/state.yml`，同步更新 README；不创建 `optimize/history/`。
...
- 模型/平台产物固定分为三类：`baseline/` 保存优化前基线，`optimize/` 保存当前优化状态与精简历史，`acceptance/` 保存最终候选验收状态。三个目录都以供 Agent 读取的 `state.yml` 和供人查看的 `README.md` 为入口，具体要求见 [模型产物契约](model-artifact-contract.md)。不得在平台根部再建立并列结果目录。
- 某个算子的环境、benchmark、patch、原始回执和逐轮过程文件保存在远程 case 目录或目标源码仓库；模型 `state.yml` 保存 revision、SHA、外部路径、结果和决策，README 保存可读结论，关键算子知识库只建立证据卡。
...
案例收尾时清理一次性文件：硬编码 PID/容器名的一次性启动、安装、停止、监控、替换和核验脚本，在其有效结论已写入 `state.yml`/README 后从模型目录移除；仍需复现且可跨模型复用的脚本必须参数化并迁入公共工具目录。瞬时机器状态只保留稳定字段和外部产物位置，不保存很快失效的 TXT dump。已完成的性能测试必须在 baseline 或 optimize README 中给出实际结果表；未得到数据时必须明确写状态、原因和下一触发条件。
...
| 2026-09-14 | 0.37 | 用户要求模型目录只保留 Agent 与人类双视图 | 将三个模型目录收敛为 `state.yml` 结构化事实和 `README.md` 人类摘要；历史只保留精简 YAML/Markdown，过程脚本、源码、patch、原始结果和 trace 外置；要求已执行性能测试必须在 README 中展示数据 | 用户指定产物可读性与精简规则 `invariant` |
| 2026-09-14 | 0.38 | 用户要求后续模型优化不保留 history 目录 | 后续每轮实验、精度问题、profiling 和规划复盘统一写入 `optimize/state.yml` 的稳定 ID 账本并同步 `README.md`；禁止新建模型内 `history/` 或逐轮目录，机制经验进入案例/知识库，原始证据外置；已有 history 由用户另行调整 | 用户指定产物结构 `invariant` |
