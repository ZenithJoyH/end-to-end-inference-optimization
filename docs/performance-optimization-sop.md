...
编译/JIT、allocator 和业务 prefix cache 分开记录。明确预热与测量请求的关系、共享前缀比例、请求顺序和每组开始前的队列排空条件；不同 seed 不等于相同缓存命中率。正式精度会改变服务历史，之后按契约恢复缓存与负载条件；涉及重启时重新满足同服务实例的精度门禁。cache 清理仅作用于任务服务，不影响共享目录或其他服务。
