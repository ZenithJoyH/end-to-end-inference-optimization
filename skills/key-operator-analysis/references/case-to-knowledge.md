...
`docs/cases/<case-id>/` 只保留少量可读摘要和证据链接；默认使用 `README.md`，确有需要时增加专题 Markdown。

对应 `models/<model>/<platform>/` 的三个子目录分别维护事实主记录：`baseline/` 保存优化前环境、workload 和测量；`optimize/` 保存源码 revision、逐轮配置、可复用命令、patch、结果摘要、失败和回退；`acceptance/` 保存最终候选的验收配置、结论、未完成状态及回退入口，中间或未达标实验数据仍留在 `optimize/`。原始日志、trace、完整结果和其他大体积产物保存在外部，只记录路径和校验信息。案例摘要与算子证据卡引用这些记录，不重复维护同一事实。
...
正文可以比 YAML 更易读，但这些语义必须齐备。性能数字只做索引摘要，并经案例摘要链接到对应实验产物；不同 workload 的百分比不能放在同一列形成虚假可比性。
...
- 实验数字可追溯到模型/平台的事实主记录及外部原始产物，案例和技能不维护另一份事实。
- case card 足以在一分钟内判断环境相似度和是否值得打开案例摘要及其证据链接。
