...
本目录保存 GPQA Diamond 精度评测工具，是端到端推理优化项目默认的正式模型级精度入口。正式精度验收固定使用：

- `llmrun.py`：基于 `lm-evaluation-harness`（命令为 `lm_eval`）的正式单服务评测入口。
- `llmrun_parallel.py`：保留用于明确指定的多服务、多 shard 场景，不能替代本项目默认的正式验收入口。

正式评测必须在目标机器上进入基于
`harbor.baai.ac.cn/flageval/flageval-llmeval:v1` 镜像的容器执行。默认评测完整的
198 道 `gpqa_diamond_generative_cot`。

评测容器是精度客户端，不是模型推理容器：最终候选 graph 服务仍运行在本次新建的优化容器内，`llmrun.py` 通过 OpenAI 兼容 endpoint 访问该服务。正式记录必须同时写明目标机器、评测容器及镜像身份、优化容器、模型名、endpoint 和网络路径。

正式流程的最低要求为：

1. 核验评测容器的 `.Config.Image` 以及 image ID/digest。
2. 为当前案例复制并冻结配置；任务必须是 `gpqa_diamond_generative_cot`，默认 `limit=0`、`expected_samples=198`。
3. 在评测容器内运行 `python3 llmrun.py <case-config.json> --preflight-only`。
4. 预检通过后，在同一评测容器内使用同一个 `llmrun.py` 和配置执行正式评测。
5. 核验 198 个唯一完整样本、`results_*.json`、`samples_*.jsonl`、effective config、超时和异常输出，再应用预先冻结的精度门槛。

阶段得分、少量样本、退出码或响应缓存都不能单独证明正式精度通过。除非用户明确要求多服务/多 shard，不能用 `llmrun_parallel.py` 替代默认的单服务正式流程。
